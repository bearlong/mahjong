...
if ($discountType == "percent") {
  if (empty($percentDiscountValue) || $percentDiscountValue <= 0) {
    $validationErrors["percentDiscountValue_error"] = "請輸入有效的優惠卷折扣值。";
  ...
  if (empty($cashDiscountValue) || $cashDiscountValue <= 0) {
    $validationErrors["cashDiscountValue_error"] = "請輸入有效的優惠卷折扣值。";
...
if (empty($limitValue) || $limitValue <= 0) {
  $validationErrors["limitValue_error"] = "請輸入有效的使用優惠卷限制金額。";
...
if (empty($usageLimit) || $usageLimit <= 0) {
  $validationErrors["usageLimit_error"] = "請輸入有效的優惠卷可使用次數。";
